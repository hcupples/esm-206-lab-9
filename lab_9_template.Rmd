...
# Note: This is bad because price per square foot is calculated forom the dependent varaible Price --- biased overfitting of model results
...
# Overall heteroscedasticity os Okay, residuals normality definittely looks good and conceptually and mathmatecially my model is making sense
...
# Based off of the AIC vlues, you would usually pick the model with the lower AIC values...(which in this case is the first model) however this model is obviously worse, and we need to actually look at the data to pcik the right model
```
...
The predict() function uses the following syntax:
      ...
      predict(data_frame, newdata = new_data_name)
